...
NHbin=[0.1,1,10,50,100] # x1e22
PhotonIndex_G = 1.9
HighECutOff = 100 # keV
redshift = arange(0,5,0.2)
#NH_24 = []
#NH_23 = []
#NH_23_5 = []
NH_24_frac = []
...
m0.zphabs.nH = 1e-2
m0.cutoffpl.PhoIndex = PhotonIndex_G
m0.cutoffpl.HighECut = HighECutOff
m0.cutoffpl.norm = 1e6
m0.zphabs.Redshift = 0
AllModels.calcFlux("5 10")
UabsFlux510 = m0.flux[0]

lstyle = itertools.cycle(["dotted","dotted","dashdot","dashed","solid"])
fig=plt.figure(1,figsize=(18,10))
fig.subplots_adjust(left=0.09, right=0.98, top=0.98,wspace=0.0, hspace=0.0)
...
for NH in NHbin:
    ...
    for Redshift in redshift:
        ...
        Ch_Cnts052 = m0.flux[0]*1.498e11
        
        AllModels.calcFlux("2 10")
        Flux210 = m0.flux[0]
        XMM_Cnts210 = m0.flux[0]*4.53E+10 
        Ch_Cnts210 = m0.flux[0]*4.291E+10 

        if abs(NH-10)<1e-4:
            AllModels.calcFlux("5 10")
            Flux510 = m0.flux[0]
            NH_23.append((XMM_Cnts210-XMM_Cnts052)/(XMM_Cnts210+XMM_Cnts052))
            NH_23_frac.append((Flux510)/UabsFlux510)
            z2_23.append(Redshift)

        if abs(NH-50)<1e-4:
            AllModels.calcFlux("5 10")
            Flux510 = m0.flux[0]
            NH_23_5.append((XMM_Cnts210-XMM_Cnts052)/(XMM_Cnts210+XMM_Cnts052))
            NH_23_5_frac.append((Flux510)/UabsFlux510)
            z2_23_5.append(Redshift)
        
        if abs(NH-100)<1e-4:
            AllModels.calcFlux("5 10")
            Flux510 = m0.flux[0]
            NH_24.append((XMM_Cnts210-XMM_Cnts052)/(XMM_Cnts210+XMM_Cnts052))
            NH_24_frac.append((Flux510)/UabsFlux510)
            z2_24.append(Redshift)
                    
        HR1_XMM.append((XMM_Cnts210-XMM_Cnts052)/(XMM_Cnts210+XMM_Cnts052))
