...
# Data
########################################################
#work = ["LaFranca", "La Franca", "Silverman 1", "Silverman 2", "Ueda", "Ueda All", "Ueda BLAGN", "Fotopoulou", "Aird1", "Aird2", "Fotopoulou uncert."]
...
#            plt.plot(Lx, Phi,ls='-.',lw=3, color ='k',label=label_d)       
            ...
            plt.plot(Lx, Phi, ls='-',lw=4, label=label_d,color=colors.next())
...
#            LF_upper = log10( interval[:, 2] )
            ...
            LF_upper = ( interval[:, 4] )
            clr='gray'
            plt.fill_between(LF_ll, LF_low, LF_upper, color=clr, alpha=0.5)
...
#    plt.savefig('/home/sotiria/workspace/Luminosity_Function/src/LF_Plots/covariance/Ueda_best_fit_LFs.'+ext)
plt.show()
