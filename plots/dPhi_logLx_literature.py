...
zB_min = [0.1, 0.5, 2.1, 3.2]
zB_max = [0.3, 0.8, 2.7, 4.0]
...
# LF grid
redshift = [0.2, 0.65, 2.4, 3.6]# np.unique(z_min_Vmax)#redshift = np.unique(z_Vmax)
...
for model in models:      
    ...
    for z in redshift:
        ...
        # model fit
        ax = fig.add_subplot(2, 2, plt_indx)
        ...
        if z>3:
            plt.plot(L, LF_Vito, color='black', ls = '-',lw=1.5, label='Vito+14: highz')
            plt.plot(L, LF_Georgakakis, color='gray', ls = '-',lw=1.5, label='Georgakakis+15: highz')
        ...
        if plt_indx in set([1, 3]): plt.yticks(visible=True)
        if plt_indx in set([3, 4]): plt.xticks(visible=True)
        ...
        if plt_indx == 3 : plt.ylabel('$\mathrm{Log[d\Phi/logL_x/(Mpc^{-3})]}$')
        if plt_indx == 3 : plt.xlabel('$\mathrm{Log[L_x/(erg/sec)]}$')

        if plt_indx == 2 : 
            plt.text(42.25, -5.95, r'$\rm{this\,work}$', size='x-small')
            plt.text(42.25, -6.30, r'$\rm{Aird+15}$', color='red', size='x-small')
            plt.text(42.25, -6.65, r'$\rm{Buchner+15}$', color='orange', size='x-small')
            plt.text(42.25, -7.00, r'$\rm{Miyaji+15}$', color='green', size='x-small')
            plt.text(42.25, -7.35, r'$\rm{Ueda+14}$', color='blue', size='x-small')
            plt.text(42.25, -7.70, r'$\rm{Hopkins+2007}$', color='magenta', size='x-small')
        
        if plt_indx == 4 : 
            plt.text(42.25, -7.00, r'$\rm{thin\,solid\,lines:}$', color='black',size='xx-small')
            plt.text(42.25, -7.35, r'$\rm{Vito+2014}$', color='black',size='x-small')
            plt.text(42.25, -7.70, r'$\rm{Georgakakis+15}$', color='gray', size='x-small')
        ...
        plt.text(xmax - (xmax-xmin)*0.3, ymax - (ymax-ymin)*0.15, r'z='+str(z), size='x-small')        

        plt_indx = plt_indx + 1

    #plt.savefig('XLF_'+model+'_literature.pdf', dpi=200)
