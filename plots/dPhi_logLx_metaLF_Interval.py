...
LF_path  = '/home/Sotiria/workspace/Luminosity_Function/src/'
param_path = '/home/Sotiria/workspace/Luminosity_Function/src/MultiNest/'

# meta-LF grid
metaLF_file = np.loadtxt(LF_path + 'meta-luminosity_function/metaLF-results/11/output_extrapolated/collated.txt')


redshift = np.unique(metaLF_file[:,1])
#print redshift

#luminosity = np.unique(metaLF_file[:,0])
luminosity = np.linspace(40, 50, 20)
# model
models = ['LDDE', 'LADE', 'ILDE', 'PDE', 'PLE']

for model in models:
    print model
    out_file = open(path + 'data/dPhi_interval_'+model+'_for_metaLF.dat', 'w')
