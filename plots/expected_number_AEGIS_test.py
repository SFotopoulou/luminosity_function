...
def plot_metaLF(color='blue'):   
    ...
    for i in xrange(0, len(Lbin)-1): 
        ...
        for lx, z in zip(meta_l, meta_z):
            area = get_area(get_flux(lx, z))
            Dv = dif_comoving_Vol(z, area)*3.40367719e-74 # Mpc^3
            PF = get_at(lx,z,level='1sigma')
            
            Nmean = Nmean + np.power(10.0, PF[0])*Dv*dz*dL
            Nmin = Nmin + np.power(10.0, (PF[0]-PF[1]))*Dv*dz*dL
            Nmax = Nmax + np.power(10.0, (PF[0]+PF[2]))*Dv*dz*dL
# Histogram per bin, find desired confidence interval        
        print Nmean, Nmin, Nmax
        N.append(Nmean)
        N_min.append(Nmin)
        N_max.append(Nmax)        
    plt.plot(Lmean, N, color=color)
    plt.fill_between(Lmean, N_min, N_max, color=color, alpha=0.2)
    plt.draw()
...
def plot_metaLF_dist(color='gray'):   
    ...
    for i in xrange(0, len(Lbin)-1): 
        ...
        for lx, z in zip(meta_l, meta_z):
            area = get_area(get_flux(lx, z))
            Dv = dif_comoving_Vol(z, area)*3.40367719e-74 # Mpc^3
            PF = get_percentiles(lx,z)
            Nmean = Nmean + (np.power(10.0, PF[50])*Dv*dz*dL) # TO FIX
...
#    plt.savefig('/home/sotiria/workspace/Luminosity_Function/output_files/plots/expected_number_AEGIS_low_z'+extension)
