...
steps = 100.
redshift = list(np.linspace(0,3,steps))
...
for z,i in zip(redshift, np.arange(len(redshift))):
    PLE = np.log10(m.PLE(Lx,z,42.9, 0.13, 2.21, 2.75, -0.5, 1.526))-4.51
    zz = z
    if zz in [0.0,redshift[17],1.0,2.0,redshift[-5]]:
...
for z,i in zip(redshift, np.arange(len(redshift))):
    PDE = np.log10(m.PDE(Lx,z,44.26, 0.63, 2.92, 3.78, -0.41, 1.22))-6.2
    zz = z
    if zz in [0.0,redshift[17],1.0,2.0,redshift[-5]]:
        print zz
...
for z,i in zip(redshift, np.arange(len(redshift))):
    ILDE = np.log10(m.ILDE(Lx,z,42.81, 0.09, 2.16, 2.74, -1.21))-4.08
    zz = z
    if zz in [0.0,redshift[17],1.0,2.0,redshift[-5]]:
...
for z,i in zip(redshift, np.arange(len(redshift))):
    LADE = np.log10(m.LADE(Lx,z,44.42, -0.03, 2.04, -1.3, 3.56, 2.3, -0.23))-3.83
    zz = z
    if zz in [0.0,redshift[17],1.0,2.0,redshift[-5]]:
...
for z,i in zip(redshift, np.arange(len(redshift))):
    Fotopoulou = np.log10(m.Fotopoulou(Lx,z,44.63, 1.4, 3.4, 4.89, -3.8, 2.03, 43.85, 0.55))-7.45
    zz = z
    if zz in [0.0,redshift[17],1.0,2.0,redshift[-5]]:
...
for ext in ['pdf','eps','png','jpg','svg']:
    plt.savefig('/home/sotiria/workspace/Luminosity_Function/src/LF_Plots/model_comparison/MLE_results_models.'+ext)
